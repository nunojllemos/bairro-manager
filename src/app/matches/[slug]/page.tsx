        ...
        <>
            ...
            <div className="flex items-center gap-x-4 pl-2 mt-12 md:mt-20">
                <span
                    className={`inline-block w-4 h-4 rounded-lg ${
                        DUMMY_GAME.isWin ? 'bg-green-500' : isDraw ? 'bg-yellow-500' : 'bg-red-500'
                    }`}
                ></span>
                ...
                <div className="grid md:grid-cols-2 gap-y-24 md:gap-x-8 mt-4">
                    <ul className="flex flex-col gap-y-2">
                        <li className="px-1 mb-4 flex items-center gap-x-4">
                            <span>Bairro</span>
                            &mdash;
                            <span className="font-semibold text-base">{DUMMY_GAME.teams.bairro.tactics}</span>
                        </li>
                                            ...
                                            <span className="flex items-center gap-x-2">
                                                ...
                                                {player?.sub && (
                                                    <span className="text-green-600">
                                                        <RestartAltOutlined fontSize="inherit" className="rotate-180" />
                                                        <span className="text-xs">
                                                            {player.sub?.min}
                                                            &apos;
                                                        </span>
                    ...
                    <ul className="flex flex-col gap-y-2">
                        <li className="px-1 mb-4 flex items-center gap-x-4">
                            ...
                            &mdash;
                            <span className="font-semibold text-base">{DUMMY_GAME.teams.other.tactics}</span>
                        </li>
                        {DUMMY_GAME.teams.other.players
                            .filter((player) => player.position !== 'sup')
                            .map((player) => {
                                return (
                                    <li key={player.name} className="flex items-center gap-x-4">
                                        <span className="border border-slate-200 rounded-md w-8 text-center py-1 text-xs">
                                            {player.position}
                                        </span>
                                        <span className={`${true}`}>{player.name}</span>
                                    </li>
                                )
                            })}
                        <li>
                        ...
                        <li className="text-xs text-slate-400">Suplentes</li>
                        {DUMMY_GAME.teams.other.players.filter((player) => player.position === 'sup').length > 0 ? (
                            DUMMY_GAME.teams.other.players
                                .filter((player) => player.position === 'sup')
                                .map((player) => {
                                    return (
                                        <li key={player.name} className="flex items-center gap-x-4 text-slate-400">
                                            <span className="border border-slate-200 rounded-md w-8 text-center py-1 text-xs">
                                                {player.position}
                                            </span>
                                            <span className={`${true}`}>{player.name}</span>
            ...
            <section>
                ...
                <div className="mt-4 py-4 px-6 border border-slate-300 rounded-md">
                    <p className="text-sm">
                        Lorem ipsum, dolor sit amet consectetur adipisicing elit. Culpa, quo accusamus. Porro
                        consequuntur ullam dolorum veritatis earum neque vero quibusdam facilis ad, temporibus saepe
                        atque ipsa, nostrum enim voluptatem ducimus! Culpa obcaecati incidunt veniam repellat magni
                        delectus sit numquam itaque repellendus omnis quas, fuga excepturi consequuntur, accusantium ex,
                        ipsam nostrum possimus. Numquam blanditiis id quis alias! Quos non qui assumenda.
                    </p>
                </div>
            </section>
            <Divider className="!my-12" />
